...
# ------------------------------------------------------------------------------
#
#    NOTE: This current version is modified by Xiaohu Hu (hux2@ornl.gov)
#
#    DATE: April, May 2009, Jan 2012
#
...
#        acticated with the option "-cmap")
#     
#    These subroutines are independent from the original charmm2lammps.pl; the 
#    original code of charmm2lammps.pl is unchanged. The new routines only
...
# Initialization
  ...
  sub Test
  {
    my $name		= shift(@_); # "@_" = argume passed to the subroutine
  ...
  sub Initialize					# initialization
  {
    ...
    $year		= "2012"; # Last modified by Xiaohu Hu, Jan 2012
...
# ------------- Reread the previously generated LAMMPS data file --------------------
	...
	die "\"sub DihedralCorrect6Ring\" can not open \"$project.data!\n";
...
# ------------ Add new dihedral types for the mis-assigned dihedrals -----------
	...
	print "\nWriting \"$project_corrected.data\"...\n\n";
...
sub CharmmCmap 
{
	...
	my @raw_data;
	open(LAMMPS, "< $project.data") or
	die "\"sub CharmmCmap()\" can not open \"$project.data!\n";
	print "Analysing \"$project.data\"...\n\n";
	@raw_data = <LAMMPS>;
	close(LAMMPS);
	...
	#
	# ---------------------------------------------------------
	#  atom  |  mass  |effect. charge|     amino-acid
	# ---------------------------------------------------------
	...
	if ($carbon_counter == 0) {
		die "No carbon atoms exist in the system, CMAP usage impossible\n";
	}
	if ($nitrogen_counter == 0) {
		die "No nitrogen atoms exist in the system, CMAP usage impossible\n";
	}
...
# need to check this part
	...
	if ( $C_counter == 0 or 
		...
		if ($C_counter == 0) {
			print "\nCan not find the peptide backbone C atom type\n";
		}
		if ($CA_counter == 0 and $CA_GLY_counter == 0 and $CA_PRO_counter == 0) {
			print "\nCan not find the peptide backbone C-alpha atom type\n";
		}
		if ($N_counter == 0 and $N_PRO_counter == 0) {
			print "\nCan not find the peptide backbone N atom type\n";
		}
...
#                         ${dihedrals_matrix[$i]}[5] == ${dihedrals_matrix[$i-1]}[5]);
	...
	# print "PHI-dihedrals: @PHI_dihedrals\n";
	# print "PSI-dihedrals: @PSI_dihedrals\n";
	...
	for (my $i = 0; $i <= $#PHI_dihedrals; $i++) {
		...
		for (my $j = 0; $j <= $#PSI_dihedrals; $j++) {
			...
			if ( ($phi2 == $psi1 and $phi3 == $psi2 and $phi4 == $psi3) or
			     ...
			     # Def. the type of the crossterm; If C_alpha of the crossterm is
			     # - ALA type, then $crossterm_type = 1;
			     # - GLY type, then $crossterm_type = 2;
			     # - PRO type, then $crossterm_type = 3;
                             # - ALA-PRO (ALA is the current AA), then $crossterm_type = 4;
                             # - PRO-PRO (PRO is the current AA), then $crossterm_type = 5;
			     ...
			     if ($crossterm_CA_charge == $charge_CA_GLY) { $crossterm_type = 2; $crossterm_type2_flag = 1; }
			     if ($crossterm_CA_charge == $charge_CA_PRO) { 
					...
					if ($crossterm_counter-1 >= 0 and $PHI_PSI_matrix[$crossterm_counter-1][0] == 2) {
                                                $PHI_PSI_matrix[$crossterm_counter-1][0] = 5;
                                                $crossterm_type5_flag = 1;
                                        }
	...
	# Checking whether the amino acid at the C-terminus is a PRO or not. If yes, the type of the last corssterm
	# should be set to 5 instead of 3!
	#
	...
	foreach $line (@raw_data) {
		...
		if ($line =~ m/dihedral types/) {
			printf(REWRITE "%12d  crossterm types\n", $ncrosstermtypes);
		}
	}
	printf(REWRITE "CMAP\n\n");
