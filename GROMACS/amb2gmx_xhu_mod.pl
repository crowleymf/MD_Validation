...
{
  my $command = "cat $crd_filename | /share/apps/amber11.tor/exe/ambpdb -atm -p $prmtop_filename > $pdb_filename";
...
{
  my $command = "/share/apps/amber11.tor/exe/rdparm $prmtop_filename < $rdparm_input_filename > $rdparm_output_filename";
  print "Calling rdparm to extract information from prmtop file.\n$command\n" if($debug_flag);
  my $output = `$command`;
  print "$output\n" if($debug_flag);
}
...
# Modified by Xiaohu Hu (hux2@ornl.gov), Feb 2011.
# Commented out the conversion to RB representation of the dihedrals, Gromacs 4.5.3 does not 
# calculate the same dihedral energy as AMBER using the same structure and FF parameters

# Use the normal dihedral representation V =K*(1+cos(n*phi))
print TOP "\n[ dihedrals ]
;  ai    aj    ak    al funct  phi  k  mult\n";
for(my $i=1; $i<=$numpropers; $i++) {
printf TOP "    %-4s %-4s %-4s %-4s %3d %12.4e %12.4e %2d\t;\n",$at{$i,1},$at{$i,2},$at{$i,3},$at{$i,4},1,$phase{$i},$pk{$i}*$cal,$pn{$i};
}
